...
import { HeaderPage, HeroPage, DevelopmentPage, ContactPage, TestSuitesPage, ChatPage } from './pages';
...
test.describe('Accessibility — WCAG 2.1 AA (axe-core)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await new HeroPage(page).freezeAnimations();
  });
  ...
  test('resume section has no WCAG 2.1 AA violations', async ({ page }) => {
    await new DevelopmentPage(page).scrollToSection('resume');
    const results = await new AxeBuilder({ page })
      .include('#resume')
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze();
...
test.describe('Accessibility — ARIA attributes and roles', () => {
  ...
  test('chat FAB aria-label and aria-expanded update when opened', async ({ page }) => {
    const chat = new ChatPage(page);
    await chat.open();
    await expect(chat.fab).toHaveAttribute('aria-label', 'Close LoganGPT');
  ...
  test('chat panel has role="dialog" and aria-modal="true"', async ({ page }) => {
    const chat = new ChatPage(page);
    await chat.open();
    await expect(chat.panel).toHaveAttribute('role', 'dialog');
    await expect(chat.panel).toHaveAttribute('aria-modal', 'true');
    await expect(chat.panel).toHaveAttribute('aria-label', 'LoganGPT chat assistant');
  });

  test('chat iframe has a descriptive title', async ({ page }) => {
    const chat = new ChatPage(page);
    await chat.open();
    await expect(chat.iframe).toHaveAttribute('title', 'LoganGPT AI Assistant');
  });

  test('close and expand buttons inside chat have aria-labels', async ({ page }) => {
    const chat = new ChatPage(page);
    await chat.open();
    await expect(chat.closeBtn).toHaveAttribute('aria-label', 'Close chat');
    await expect(chat.expandBtn).toHaveAttribute('aria-label', 'Expand chat');
  });
  ...
  test('sections dropdown trigger has aria-haspopup', async ({ page }) => {
    ...
    await expect(header.sectionsMenuTrigger).toHaveAttribute('aria-haspopup', 'menu');
  });

  test('development section has aria-label', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await expect(dev.section).toHaveAttribute('aria-label', 'Development projects');
  });

  test('carousel dots use role="tablist" and role="tab"', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    await expect(dev.carouselDots()).toHaveAttribute('role', 'tablist');
    await expect(dev.carouselDots().locator('.dot').first()).toHaveAttribute('role', 'tab');
  });

  test('carousel arrows have descriptive aria-labels', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    await expect(dev.carouselPrevArrow()).toHaveAttribute('aria-label', 'Previous image');
    await expect(dev.carouselNextArrow()).toHaveAttribute('aria-label', 'Next image');
  });
  ...
  test('deep-dive toggle has aria-controls pointing to panel id', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    await expect(dev.deepDiveToggle('logangpt')).toHaveAttribute('aria-controls', 'deep-dive-logangpt');
  });
  ...
  test('expanded deep-dive panel has role="region" with aria-label', async ({ page }) => {
    ...
    await dev.scrollToSection();
    await dev.expandProject('logangpt');
    const panel = dev.deepDivePanel('logangpt');
  ...
  test('tech stack containers have aria-label="Technologies used"', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    await expect(page.locator('[aria-label="Technologies used"]').first()).toBeVisible();
  });
...
test.describe('Accessibility — Keyboard navigation', () => {
  ...
  test('brand logo is reachable by Tab', async ({ page }) => {
    ...
    await expect(header.brandLogo).toBeFocused();
  });
  ...
  test('sections dropdown trigger is reachable by Tab after brand logo', async ({ page }) => {
    ...
    await expect(header.sectionsMenuTrigger).toBeFocused();
  });
  ...
  test('resume header link is reachable by Tab after sections trigger', async ({ page }) => {
    ...
    await expect(header.resumeLink).toBeFocused();
  });
  ...
  test('sections dropdown opens and closes with keyboard', async ({ page }) => {
    ...
    await expect(header.menu).not.toBeVisible();
  });

  test('chat FAB is reachable by keyboard and toggles with Enter', async ({ page }) => {
    const chat = new ChatPage(page);
    await chat.fab.focus();
    await expect(chat.fab).toBeFocused();
    await page.keyboard.press('Enter');
    await expect(chat.openPanel).toBeVisible();
  });
  ...
  test('carousel arrows are keyboard-operable', async ({ page }) => {
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    const nextArrow = dev.carouselNextArrow();
    await nextArrow.focus();
    await expect(nextArrow).toBeFocused();
    await expect(dev.carouselDots().locator('.dot').first()).toHaveAttribute('aria-selected', 'true');
    await page.keyboard.press('Enter');
...
test.describe('Accessibility — Resilience and edge cases', () => {
  ...
  test('carousel prev-arrow wraps correctly from first to last slide', async ({ page }) => {
    ...
    await dev.carouselPrevArrow().click();
    await expect(dev.carouselDots().locator('.dot').nth(totalDots - 1)).toHaveAttribute('aria-selected', 'true');
  });
  ...
  test('multiple rapid carousel clicks do not break aria-selected state', async ({ page }) => {
    ...
    const dev = new DevelopmentPage(page);
    await dev.scrollToSection();
    for (let i = 0; i < 5; i++) await dev.carouselNextArrow().click();
  ...
  test('only one deep-dive panel is open and aria-expanded is consistent', async ({ page }) => {
    ...
    await expect(dev.deepDiveToggle('logangpt')).toHaveAttribute('aria-expanded', 'false');
    await expect(dev.deepDiveToggle('math-trainer')).toHaveAttribute('aria-expanded', 'true');
  });
